...
In this paper we outlined the details of a distributed version of the SMHA* algorithm. We detailed the structure of the algorithm, and presented experimental results comparing the performance of the algorithm while varying a number of parameters. We also compared its performance to the original SMHA* algorithm. Although we found that in the specific domain we tested on (the sliding tile puzzle) our distributed variant did not outperform the original algorithm, we provided evidence that our algorithm could outperform SMHA* in domains where heuristics are difficult to calculate, or in general where state expansion is expensive.
% TODO: Fill me in!
