...
\textbf{Back-pointer optimization}

$bp(s)$ is a pointer to the parent of $s$ along the optimal path found so far from the start to $s$. It is not needed to perform the search and compute a conservative distance to the goal; it is only used to reconstruct the path once the search terminates.
